...
if sys.platform == "linux":
    ...
    tmpPath = localPath + "/cyDrivers"
    sys.path.insert(0, tmpPath)
    # from EmotivBTLE import *
else:
    from ctypes import *
...
class EEG(object):
    ...
    def Setup(self, model, config):
        ...
        if sys.platform == "linux":
            deviceList = ['Emotiv', 'Brain Computer Interface USB Receiver/Dongle', '0x1234', '0xed02']
        elif sys.platform == "win32":
        ...
        if eeg_driver == "pyusb" and self.device == None:
            try:
                ...
                if sys.platform == "linux":
                    ...
                    rootPath = "./cyDrivers/" + str(machine) + "/" + str(os_platform)

                    for file in os.listdir(rootPath):
                        if file.startswith("libusb"):
                            path = os.path.join(rootPath, file)

                    backend = cyPyUSB.backend.libusb1.get_backend(find_library=lambda x: path)
                elif sys.platform == "win32":
                        ...
                        if product == deviceList[i]:
                            ...
                            if useDevice.upper() == "Y":
                                ...
                                if threadMax < 2:
                                    ...
                                    ep = cyPyUSB.util.find_descriptor(intf, custom_match=lambda
                                        e: cyPyUSB.util.endpoint_direction(
                                        e.bEndpointAddress) == cyPyUSB.util.ENDPOINT_IN)
                                    if eval(self.cyIO.getInfo("verbose")) == True:
                                        mirror.text(
                                            f" bEndpointAddress: {hex(ep.bEndpointAddress)}\n wMaxPacketSize: {ep.wMaxPacketSize}")
                                    self.cyIO.setInfo("ep", ep)
                                    """
                                    detail_info = None
                                    intf = cfg[(0,0)]
                                    while 1:
                                        ...
                                        #time.sleep(.1)
                                    """
                                    if sys.platform == "linux":  # Linux makes crash calls to ctrl_transfer
                                        device_firmware = "N/A"
                                        software_firmware = "N/A"
                                        detail_info = list()
                                    else:
                                        while detail_info == None:
                                    ...
                                    if eval(self.cyIO.getInfo("verbose")) == True:
                                        mirror.text(" Detail Info = " + str(list(detail_info)))
                                        mirror.text(" Device Firmware = " + device_firmware)
                                        mirror.text(" Software Firmware = " + software_firmware)
        ...
        while self.running:
            ...
            if eeg_driver == "pyusb" and self.blankdata == False:
                ...
                try:
                    ...
                    # 0x60
                    task = self.device.read(ep.bEndpointAddress, ep.wMaxPacketSize, 100)
                    tasks.put(task.tostring())

                except Exception as e:
                    # Read Timeout. (Device Not Turned On or USB read() call too fast (1000 as timeout works fine))
                    if e.errno == 10060 or e.errno == 110:
            ...
            while not tasks.empty() and self.running == True:
                ...
                if eeg_driver == "pyusb":
                    ...
                    if self.blankdata == False:
                        try:
                            task = self.device.read(ep.bEndpointAddress, ep.wMaxPacketSize, 500)
                            tasks.put(task.tostring())
                        except Exception as e:
